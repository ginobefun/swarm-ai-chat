...
export abstract class BaseAgentNode {
    ...
    async process(state: OrchestratorState): Promise<Partial<OrchestratorState>> {
        ...
        console.log(`🔍 ${this.capability.name} (${this.capability.agentId}) checking for tasks:`, {
            inFlightTasks: Object.values(state.inFlight).length,
            myTasks: myTasks.length,
            taskAssignments: Object.values(state.inFlight).map(t => ({ taskId: t.id.substring(0, 8), assignedTo: t.assignedTo }))
        })

        if (myTasks.length === 0) {
            console.log(`⚠️ ${this.capability.name} found no tasks assigned to agentId: ${this.capability.agentId}`)
            return {}
        }
